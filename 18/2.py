...
class Node:
    def __init__(self, left, right, parent: Optional['Node'] = None, depth:int = 0):
        self.left = left
        self.right = right
        self.parent = parent
    ...
    def __deepcopy__(self, memodict={}):
        deep_left = self.left.__deepcopy__() if isinstance(self.left, Node) else self.left
        deep_right = self.right.__deepcopy__() if isinstance(self.right, Node) else self.right
        return Node(deep_left, deep_right, depth=self.depth)
...
def add(a: Node, b: Node) -> Node:
    if verbose > 0:
        ...
        print(b)
        print("=")
    new_root = Node(deepcopy(a), deepcopy(b))
...
fishsums = []
for fish_a in fishints:
    for fish_b in fishints:
        if fish_a != fish_b:
            fishsums.append((fish_a, fish_b, add(fish_a, fish_b)))

max_sum = max(fishsums, key=lambda s: s[2].magnitude())
print(f"Max Fish: {max_sum[0]} + {max_sum[1]} = {max_sum[2]}")
print(f"Max Magnitude: {max_sum[2].magnitude()}")
